...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name="Titanic_Captum_mlflow2.2"):
        ...
        #Exemplo de uma assinatura escrita manualmente:
        # input_schema = Schema(
        # [
        ...
        signature = infer_signature(train_features) #Inferindo a assinatura do modelo. Você também pode montar a assinatura de forma manual.
        


        
        mlflow.pytorch.log_model(net, "model", signature = signature)  #Salva o modelo com a assinatura definida. A assinatura do modelo é opcional
        mlflow.log_param("Train Size", len(train_labels))
        mlflow.log_param("Test Size", len(test_labels))
        mlflow.log_artifact(local_path="./Titanic_Captum_interpret.py", artifact_path="code") #salvando um artefato, no caso estamos salvando esse código
        
        model_path = mlflow.get_artifact_uri("model") #Aqui o programa está pegando o caminho do modelo salvo pelo proprio MLflow
        loaded_pytorch_model = mlflow.pytorch.load_model(model_path)  # Aqui o programa carrega o modelo salvo pelo proprio MLflow para utiliza-lo para fazer predições de teste (incrivel :0)
        
        prediction = loaded_pytorch_model(torch.from_numpy(test_features).type(torch.FloatTensor)) #fazendo a predição utilizando o proprio modelo salvo pelo MLflow. A predição está sendo feita com a propria base de treino, só para fins de teste mesmo
